...
while not os.path.exists(f'{args.datadir}/regtest/.cookie'):
    time.sleep(0.5)

# Read .cookie file to get user and pass
with open(f'{args.datadir}/regtest/.cookie') as f:
    userpass = f.readline().lstrip().rstrip()
rpc = AuthServiceProxy(f'http://{userpass}@127.0.0.1:18443')
...
def_rpc = AuthServiceProxy(f'http://{userpass}@127.0.0.1:18443/wallet/')
big_rpc = AuthServiceProxy(f'http://{userpass}@127.0.0.1:18443/wallet/big')
